...
def test_get_song_list(client):
    resp = client.get(f"/songs/")
...
@pytest.mark.usefixtures('populate_artist_data', "populate_song_data")
def test_get_song_resource(client):
    resp = client.get(f"/songs/1")
...
def test_create_song_with_artist_resource(client):
